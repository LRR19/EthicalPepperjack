...
@app.route('/recipesearch', methods=['GET'])
def search_for_recipe():
    ...
    #   Convert result tuple to list and then just get the first element of
    #   the tuple
    ingredient_list = list(execute_query(query))
    #    ingredient_list =[item for t in result for item in t]
    #   Pass the search query and the list of ingredients to the new html for
    #   display.
...
# 	# Sample query
# 	# query = """SELECT * FROM table JOIN table.id = table2.id WHERE
# 	table.id = %d;""" %(table_id)
